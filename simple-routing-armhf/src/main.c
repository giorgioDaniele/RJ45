...
{	
	...
	struct bpf_tc_hook *hooks;
	struct bpf_tc_opts *hopts;
	
	printf("############################################\n");
	printf("            Welcome to RJ45 router 		    \n");
	...
	while (exiting)
	{
		...
		if (strlen(cmd) == 0)
	       	{
			printf("# No valid command. Hit \"help\"\n");
		}
		else if (strcmp(cmd, start_cmd) == 0)
		{
			...
			/**
			 * Esecuzione di bpftool net list
			*/

			//system("tools/bpftool net list");
		...
		else if (strcmp(cmd, list_cmd) == 0) 
		{
			...
			for (i=0; i<n; i++)
			{
				printf("\tInterface Name=%s Index=%u\n", 
						interfaces[i], if_nametoindex(interfaces[i]));
			}
		}
